...
export default function HomePage() {
  ...
  async function loadData(){
    ...
    setEquipment((eq.data ?? []) as Equipment[]); setOrders((ord.data ?? []) as unknown as Order[]); setPickups((pick.data ?? []) as Pickup[]); setVisits((visit.data ?? []) as Visit[])
  }
  ...
  const availableUnits=useMemo(()=>equipment.reduce((s,x)=>s+x.available_qty,0),[equipment]); const scheduledDeliveries=orders.filter(x=>x.status==='scheduled').length; const newOrders=orders.filter(x=>x.status==='new').length
  ...
  return <div className="app-shell"><header className="topbar"><div className="brand"><Logo className="brand-logo"/></div><div className="top-actions"><button className="btn btn-primary" onClick={()=>setTab('new-order')}>+ New Order</button><button className="btn btn-dark desktop" onClick={signOut}>Sign Out</button></div></header><main className="container"><div className="hero"><div><h2>{tab==='home'?'Today':tab==='inventory'?'Inventory':tab==='orders'?'Orders':'New Rental Order'}</h2><p>{new Date().toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric',year:'numeric'})}</p></div></div><nav className="nav"><button className={tab==='home'?'active':''} onClick={()=>setTab('home')}>Home</button><button className={tab==='inventory'?'active':''} onClick={()=>setTab('inventory')}>Inventory</button><button className={tab==='orders'?'active':''} onClick={()=>setTab('orders')}>Orders</button><button className={tab==='new-order'?'active':''} onClick={()=>setTab('new-order')}>Place Order</button></nav>{tab==='home' && <Dashboard deliveries={scheduledDeliveries} pickups={pickups.length} visits={visits.length} available={availableUnits} newOrders={newOrders} recentOrders={orders.slice(0,6)} go={setTab}/>} {tab==='inventory' && <Inventory equipment={equipment} onQty={updateQty}/>} {tab==='orders' && <Orders orders={orders}/>} {tab==='new-order' && <NewOrder equipment={equipment} user={user} onSaved={async()=>{await loadData();setTab('orders')}} saving={saving} setSaving={setSaving}/>}</main><div className="bottom-nav"><button onClick={()=>setTab('home')}>Home</button><button onClick={()=>setTab('inventory')}>Inventory</button><button onClick={()=>setTab('orders')}>Orders</button><button onClick={()=>setTab('new-order')}>New Order</button></div></div>
}

function Dashboard({deliveries,pickups,visits,available,newOrders,recentOrders,go}:{deliveries:number;pickups:number;visits:number;available:number;newOrders:number;recentOrders:Order[];go:(t:Tab)=>void}){return <><div className="grid"><button className="card" onClick={()=>go('orders')} style={{textAlign:'left'}}><div className="muted">Scheduled Deliveries</div><div className="metric">{deliveries}</div></button><div className="card"><div className="muted">Scheduled Pickups</div><div className="metric">{pickups}</div></div><div className="card"><div className="muted">Planned Visits</div><div className="metric">{visits}</div></div><button className="card" onClick={()=>go('inventory')} style={{textAlign:'left'}}><div className="muted">Available Equipment</div><div className="metric">{available}</div></button><button className="card" onClick={()=>go('orders')} style={{textAlign:'left'}}><div className="muted">New Orders</div><div className="metric">{newOrders}</div></button><button className="card" onClick={()=>go('new-order')} style={{textAlign:'left'}}><div className="muted">Quick Action</div><div className="metric" style={{fontSize:22}}>Place Rental Order</div></button></div><div className="section-title"><h3>Recent Orders</h3></div><Orders orders={recentOrders}/></>}
